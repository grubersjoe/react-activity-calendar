...
import { range, validateActivities, validateLevels } from './calendar'
...
describe('validateLevels', () => {
  ...
  ] satisfies Array<[string, Levels]>)('should throw error for invalid input: %s', (_, levels) => {
    expect(() => {
  ...
  it.each([
    ...
    ['mixed', { min: -5, max: 5 }],
  ] satisfies Array<[string, Levels]>)('should accept valid input: %s', (_, levels) => {
    expect(() => {
      validateLevels(levels)
    }).not.toThrow()
  })
